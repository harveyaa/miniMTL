...
class MPSModel(nn.Module):
    """ Middle Parameter Sharing."""
    def __init__(self,preencoders,encoder,decoders,loss_fns):
        """
        Parameters
        ----------
        preencoders: dict[str, nn.Module]
            Dictionary from task name to task-specific preencoder).
        encoder: nn.Module
        ...
        self.to(self.device)
        print(f'Initialized MPSModel using: {self.device}.\n')
    ...
    def forward(self,X,task_names):
        ...
        for task in task_names:
            X_ = self.preencoders[task](X)
            X_ = self.encoder(X_)
            outputs[task] = self.decoders[task](X_)
        return outputs
